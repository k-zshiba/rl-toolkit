...
Issue #22 adds a GitHub Pages viewer for `rl-coach` outputs.

- The web app runs fully client-side as static HTML/CSS/JavaScript.
- Upload `analysis/*.json` or `analysis/summary.json` generated by `rl-coach` and inspect the results in the browser.
- Replay harvesting is intentionally excluded from Pages because BallChasing API access is not suitable for a static site.

Local development for the Pages app:
