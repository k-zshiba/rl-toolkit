...
fn full_soccar_fixture_reports_full_quality_and_diagnoses() -> Result<()> {
    ...
    let labels: Vec<_> = report.concede_diagnoses[0]
        ...
        .collect();
    assert!(labels.iter().any(|label| format!("{:?}", label).contains("FailedClear")));
    assert!(labels.iter().any(|label| format!("{:?}", label).contains("DemoDisruption")));
    Ok(())
}
...
fn header_only_fixture_marks_derived_metrics_unavailable() -> Result<()> {
    ...
    assert_eq!(blue.metrics["avg_speed"].quality, MetricQuality::Unavailable);
    assert!(report
        .warnings
        .iter()
...
fn out_of_order_boost_component_link_still_computes_avg_boost() -> Result<()> {
    let dir = tempdir()?;
    let report = analyze_file(fixture_path("out_of_order_boost_link.json"), dir.path(), true)?;
...
fn analyze_directory_writes_summary_and_loads_reports() -> Result<()> {
    ...
    assert!(output_dir.path().join("analysis").join("summary.json").exists());
...
fn cli_supports_single_file_and_directory_inputs() -> Result<()> {
    ...
    let output = Command::new(binary)
        ...
        .output()?;
    assert!(output.status.success());
    assert!(dir_output.path().join("analysis").join("summary.json").exists());
    Ok(())
}
...
fn full_report_snapshot_matches_expected_schema() -> Result<()> {
    ...
    let expected: Value = serde_json::from_str(&fs::read_to_string(fixture_path("expected_full_report.json"))?)?;
